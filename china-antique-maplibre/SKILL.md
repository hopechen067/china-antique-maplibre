...
description: China antique parchment MapLibre map engine with configurable basemap, Terrarium hillshade, optional user-supplied water overlay, live tuner page, and tiered settlement extrusions for HyperFrames historical videos.
---
...
- **Water data is not shipped** — do not expect or commit `tuner/assets/water-data.js`. Optional local packs only; see repo root `DATA-PROVENANCE.md` and [`references/water-overlay.md`](references/water-overlay.md).
...
- [`references/water-overlay.md`](references/water-overlay.md) — optional `CHINA_WATER_DATA` schema & labels
